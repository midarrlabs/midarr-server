defmodule MediaServerWeb.StreamLive.Show do
  ...
  def handle_event("publish_stream", _, socket) do
    ...
    task = Task.async(fn ->
      Rambo.run(System.find_executable("ffmpeg"), ["-i", "#{socket.assigns.file.path}", "-map", "0:0", "-map", "0:1", "-c:v", "libx264", "-preset", "veryfast", "-b:v", "3000k", "-maxrate", "3000k", "-bufsize", "6000k", "-pix_fmt", "yuv420p", "-g", "50", "-c:a:1", "acc", "-b:a", "160k", "-ac", "2", "-ar", "44100", "-f", "rtsp", "#{System.get_env("RTSP_SERVER_URL") || "rtsp://rtsp-simple-server:8554"}/#{uuid}"])
    end)
