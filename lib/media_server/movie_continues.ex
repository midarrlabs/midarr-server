defmodule MediaServer.MovieContinues do
  ...
  schema "movie_continues" do
    belongs_to :movie, MediaServer.Movies, foreign_key: :movie_id
  ...
  def changeset(continue, attrs) do
    continue
    |> cast(attrs, [:movie_id, :current_time, :duration, :user_id])
    |> validate_required([:movie_id, :current_time, :duration, :user_id])
  end

  def insert_or_update(attrs) do
    case Repo.get_by(__MODULE__, movie_id: attrs.movie_id, user_id: attrs.user_id) do
      nil ->
        %__MODULE__{
