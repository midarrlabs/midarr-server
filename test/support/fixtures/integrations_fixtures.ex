defmodule MediaServer.IntegrationsFixtures do
  ...
  def add_movie(integration) do
    case HTTPoison.get("#{ integration.base_url }/api/v3/movie?apiKey=#{ integration.api_key }") do
  ...
  def radarr_fixture() do
    radarr = %{
      base_url: System.get_env("RADARR_BASE_URL"),
      api_key: System.get_env("RADARR_API_KEY")
    }
  ...
  def add_series_root(integration) do
    HTTPoison.post!("#{ integration.base_url }/api/v3/rootFolder?apikey=#{ integration.api_key }", Jason.encode!(%{
      path: "/shows"
    }))
  end

  def add_series(integration) do
    case HTTPoison.get("#{ integration.base_url }/api/v3/series?apikey=#{ integration.api_key }") do
      ...
      {:ok, %HTTPoison.Response{status_code: 200, body: body}} ->
        ...
        if length(decoded) !== 0 do
          ...
          case HTTPoison.get("#{ integration.base_url }/api/v3/episode?seriesId=#{ series["id"] }&apikey=#{ integration.api_key }") do
  ...
  def sonarr_fixture() do
    sonarr = %{
      base_url: System.get_env("SONARR_BASE_URL"),
      api_key: System.get_env("SONARR_API_KEY")
    }
