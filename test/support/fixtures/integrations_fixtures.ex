defmodule MediaServer.IntegrationsFixtures do
  ...
  def add_series_root(integration) do
    HTTPoison.post!("#{ integration.url }/api/v3/rootFolder?apikey=#{ integration.api_key }", Jason.encode!(%{
      path: "/shows"
    }))
  end

  def add_series(integration) do
    case HTTPoison.get("#{ integration.url }/api/v3/series?apikey=#{ integration.api_key }") do
      ...
      {:ok, %HTTPoison.Response{status_code: 200, body: body}} ->
        ...
        if length(decoded) !== 0 do
          ...
          case HTTPoison.get("#{ integration.url }/api/v3/episode?seriesId=#{ series["id"] }&apikey=#{ integration.api_key }") do
  ...
  def add_movie(integration) do
    case HTTPoison.get("#{ integration.url }/api/v3/movie?apiKey=#{ integration.api_key }") do
