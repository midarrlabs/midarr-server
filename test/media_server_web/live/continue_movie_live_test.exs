defmodule MediaServerWeb.ContinueMovieLiveTest do
  ...
  test "it should watch", %{conn: conn, user: _user} do
    ...
    {:ok, view, _disconnected_html} =
      live(conn, Routes.continue_movie_show_path(conn, :show, movie["id"]))
  ...
  test "it should not subtitle", %{conn: conn, user: _user} do
    {:ok, _view, disconnected_html} = live(conn, Routes.continue_movie_show_path(conn, :show, 2))
